...
/**
 * Class to handle checking the SuperLotto Plus lottery.
 *
 * @author David Lerner
 */
public class SuperLottoPlus extends Lottery
{
    public static final String MESSAGE = "SuperLotto Plus draws take place every Wednesday and Saturday after the draw entry closes at 7:45 p.m.";
    private static final String dataURL = "http://www.calottery.com/sitecore/content/Miscellaneous/download-numbers/?GameName=superlotto-plus&Order=No";
    private static final String refererURL = "http://www.calottery.com/play/draw-games/superlotto-plus/winning-numbers";
    private ArrayList<HashSet<Integer>> numbers;
    private ArrayList<Integer> superLottoPlus;

    public SuperLottoPlus() throws Exception
    {
        super();
    ...
    private void parseText(String textInput)
    {
        ...
        while (s.hasNextLine())
        {
            ...
            for (int i = 0; i < 5; i++)
                set.add(s.nextInt());
            numbers.add(0, set);
            superLottoPlus.add(0, s.nextInt());
            s.nextLine();
        }
    }
    ...
    public String[] CheckIfWon (int a[])
    {
        ...
        else
        {
            ...
            if (superLottoPlus.get(index) == a[6])
                message[1] += "\nYou matched the MEGA number!";
            else
                message[1] += "\nYou did not match the MEGA number";
            message[0] = getPrizeMoney(index, matches, (superLottoPlus.get(index) == a[6]));
        }
    ...
    private String getPrizeMoney(int index, int matches, boolean superLottoPlusMatches)
    {
        try
        {
            ...
            int numb = superLottoPlusMatches ? 0 : 1;
